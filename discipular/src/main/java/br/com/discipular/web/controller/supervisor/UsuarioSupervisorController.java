...
import br.com.discipular.web.validator.UsuarioValidator;
...
public class UsuarioSupervisorController  extends AbstractAdminController {
	...
	public @ResponseBody ModelAndView index() {
		...
		Page<Usuario> registros = service.getRepositorio().findAll(UsuarioPredicate.buscarTipoEArea(TipoUsuario.LIDER, getCurrentUser().getArea()), UsuarioPredicate.buscarPaginacao(0, QUANTIDADE_ELEMENTOS_POR_PAGINA));
		
		registros.getContent().forEach(u ->  {
			List<Celula> celula = (List<Celula>) celulaService.getRepositorio().findAll(CelulaPredicate.buscarPorLider(u.getId()));
	...
	public ModelAndView excluir(@PathVariable ("id") Long id, RedirectAttributes redirect) {
		...
		try {
			...
			List<Celula> celulas = (List<Celula>) this.celulaService.getRepositorio().findAll(CelulaPredicate.buscarPorLider(id));
	...
	public ModelAndView apiPrevious() {
		...
		Page<Usuario> registros = service.getRepositorio().findAll(UsuarioPredicate.buscarTipoEArea(TipoUsuario.LIDER, getCurrentUser().getArea()), UsuarioPredicate.buscarPaginacao(--marker, QUANTIDADE_ELEMENTOS_POR_PAGINA));
		registros.getContent().forEach(u -> u.setCelula(((List<Celula>) celulaService.getRepositorio().findAll(CelulaPredicate.buscarPorLider(u.getId()))).get(0).getNome()));
		view.addObject("registros", registros.getContent());
	...
	public ModelAndView apiFind(@PathVariable ("condicao") String nome) {
		...
		Page<Usuario> registros = service.getRepositorio().findAll(UsuarioPredicate.buscarPorNomeComFiltroTipoEArea(nome, TipoUsuario.LIDER, getCurrentUser().getArea()), UsuarioPredicate.buscarPaginacao(0, QUANTIDADE_ELEMENTOS_POR_PAGINA));
	...
	public ModelAndView resetarSenha(@PathVariable ("id") Long id, RedirectAttributes redirect) {
		...
		try {
			Usuario usuario = service.getRepositorio().findOne(id);
			usuario.setSenha(new BCryptPasswordEncoder().encode(usuario.getLogin() + "123"));
