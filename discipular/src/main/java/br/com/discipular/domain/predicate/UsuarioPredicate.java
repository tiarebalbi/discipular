...
public class UsuarioPredicate {
	...
	public static Predicate buscarPorNomeComFiltroTipoEArea(String nome, TipoUsuario tipo, int area) {
		QUsuario condicao = QUsuario.usuario;
		return condicao.tipo.eq(tipo).and(condicao.area.eq(area)).and(condicao.nome.startsWithIgnoreCase(nome).and(condicao.nome.containsIgnoreCase(nome)));
	}
