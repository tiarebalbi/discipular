	...
	if(response.registros.length > 0) {
		$(response.registros).each(function (index, value) {
			$('tbody').append("<tr>" +
					...
					'<li><a href="' + urlBase + 'admin/lider/editar/' + value.id + '"><i class="fa fa-pencil-square-o"></i> Editar</a></li>' +
					...
					'<a href="' + urlBase + 'admin/lider/alterar-senha/' + value.id + '" class="btn btn-danger">Resetar</a></div></div></div></div>' +
					...
					'<a href=" ' + urlBase + 'admin/lider/excluir/ '+ value.id + '" class="btn btn-danger">Excluir</a> </div> </div> </div> </div>' +
					"</td>" +
					"</tr>");
		});
	}  else {
