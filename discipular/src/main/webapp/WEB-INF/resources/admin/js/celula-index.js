	...
	if(response.registros.length > 0) {
		$(response.registros).each(function (index, value) {
			...
			$('tbody').append("<tr>" +
					...
					+ '<li><a href="' + urlBase + modulo + '/editar/' + value.id + '"><i class="fa fa-pencil-square-o"></i> Editar</a></li><li class="divider"></li>'
					+ '<li><a data-toggle="modal" data-target="#excluir-' + value.id + '"><i class="fa fa-trash-o"></i> Excluir</a></li>'
					+ '</ul></div>' 
					+ '<div class="modal fade" id="excluir-' + value.id + '" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">'
					...
					+ '</button>'
					+ '<h4 class="modal-title" id="myModalLabel"><i class="fa fa-warning"></i> Atenção!</h4></div><div class="modal-body"><p>Você realmente deseja excluir a célula ' + value.nome + '?</p>' 
