	...
	if(response.registros.length > 0) {
		$(response.registros).each(function (index, value) {
			$('tbody').append("<tr>" +
					...
					'<div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button><a href="' + urlBase + 'admin/supervisor/alterar-senha/' + value.usuario.id + '" class="btn btn-danger">Resetar</a></div></div></div></div>' +
