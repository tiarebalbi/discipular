	...
	if(response.registros.length > 0) {
		$(response.registros).each(function (index, value) {
			...
			$('tbody').append("<tr>" +
					...
					'<ul class="dropdown-menu" role="menu"><li><a href="' + urlBase + 'admin/celula/editar/' + value.id + '"><i class="fa fa-pencil-square-o"></i> Editar</a></li><li class="divider"></li>' +
					...
					'</div><div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button><a href="' + urlBase + 'admin/celula/excluir/' + value.id + '" class="btn btn-danger">Excluir</a></div></div></div></div>' +
